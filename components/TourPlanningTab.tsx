...
const TourPlanningTab: React.FC<TourPlanningTabProps> = ({ state, setState, customEquipment, equipmentOverrides }) => {
    ...
    const handleExportClusterWWB = (clusterId: string, eqKey?: string) => {
        ...
        const constantFreqs = cluster.constantFrequencies || [];
        const localFreqs = cluster.localFrequencies || [];
        let allFreqs = [...constantFreqs, ...localFreqs];

        if (eqKey) {
            allFreqs = allFreqs.filter(f => f.equipmentKey === eqKey);
        }
        ...
        const filename = `cluster_${cluster.name.replace(/\s+/g, '_')}_wwb_${new Date().toISOString().slice(0, 10)}${eqKey ? `_${eqKey}` : ''}`;
                    ...
                    {(!isCalculating && (state.constantSystems.frequencies?.length > 0 || state.clusters.some(c => c.localFrequencies?.length > 0))) && (
                        <>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            {openExportMenuId === 'global' && (
                                                <div className="absolute top-full right-0 mt-2 bg-slate-800 border border-indigo-500/40 rounded-xl shadow-2xl z-[120] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200 divide-y divide-white/5 min-w-[180px]">
                            ...
                            {state.clusters.map(cluster => {
                                ...
                                // Get unique equipment keys for export
                                const uniqueEqKeys = Array.from(new Set(allFreqs.map(f => f.equipmentKey).filter(Boolean)));
                                    ...
                                    <Card key={cluster.id}>
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="flex gap-1 bg-slate-900/50 p-1 rounded-lg border border-white/5">
                                                    <button 
                                                        onClick={() => toggleAllClusterLocks(cluster.id, true)}
                                                        className="p-1.5 hover:bg-emerald-500/20 rounded transition-all text-slate-500 hover:text-emerald-400 border border-transparent hover:border-emerald-500/30"
                                                        title="Lock All"
                                                    >
                                                        <Lock size={12} />
                                                    ...
                                                    <button 
                                                        onClick={() => toggleAllClusterLocks(cluster.id, false)}
                                                        className="p-1.5 hover:bg-rose-500/20 rounded transition-all text-slate-500 hover:text-rose-400 border border-transparent hover:border-rose-500/30"
                                                        title="Unlock All"
                                                    >
                                                        <Unlock size={12} />
                                                ...
                                                <div className="relative">
                                                    <button 
                                                        onClick={() => setOpenExportMenuId(openExportMenuId === cluster.id ? null : cluster.id)}
                                                        className={`${secondaryButton} flex items-center gap-1`}
                                                    >
                                                        Export WWB <span className="text-[8px] opacity-60">▼</span>
                                                    </button>
                                                    
                                                    {openExportMenuId === cluster.id && (
                                                        <div className="absolute top-full right-0 mt-2 bg-slate-800 border border-indigo-500/40 rounded-xl shadow-2xl z-[120] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200 divide-y divide-white/5 min-w-[180px]">
                                                            <div className="bg-indigo-500/15 p-2">
                                                                <div className="px-2 py-1.5 flex items-center gap-2 mb-2 border-b border-indigo-500/20">
                                                                    <span className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.1em]">WWB Export</span>
                                                                ...
                                                                <div className="flex flex-col gap-1.5 max-h-[300px] overflow-y-auto custom-scrollbar">
                                                                    <button 
                                                                        onClick={() => handleExportClusterWWB(cluster.id)}
                                                                        className="w-full text-left px-3 py-2.5 hover:bg-indigo-600 rounded bg-indigo-500/30 text-[9px] font-black text-white uppercase tracking-tighter transition-all border border-indigo-400/20 shadow-sm"
                                                                    >
                                                                        &bull; Full Cluster List
                                                                    </button>
                                                                    {uniqueEqKeys.map(key => {
                                                                        const profile = db[key as string];
                                                                        const count = allFreqs.filter(f => f.equipmentKey === key).length;
                                                                        return (
                                                                            <button 
                                                                                key={key}
                                                                                onClick={() => handleExportClusterWWB(cluster.id, key as string)}
                                                                                className="w-full text-left px-3 py-2.5 hover:bg-slate-700 rounded bg-slate-950/60 border border-white/10 text-[9px] font-bold text-indigo-200 uppercase tracking-tighter transition-all"
                                                                            >
                                                                                &bull; {profile?.name || key} ({count} CH)
                                                                            </button>
