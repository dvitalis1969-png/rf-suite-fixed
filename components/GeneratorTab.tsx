                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    <div className="md:col-span-2">
                                        ...
                                        <select value={req.key} onChange={e => handleUpdateRequest(Number(req.id), 'key', e.target.value)} className="w-full bg-slate-800 border border-blue-500/30 rounded-lg p-2.5 text-slate-200 text-sm">
                                            ...
                                            <optgroup label="Other Professional">{otherStandardProfiles.map(([key, p]) => <option key={key} value={key}>{p.name} ({p.band})</option>)}</optgroup>
                                        </select>
                                    </div>
                                    <div>
                                    ...
                                    {req.useManualParams ? (
                                        ...
                                        <div className="grid grid-cols-3 gap-3 opacity-80">
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        <div className="flex flex-col"><label className="text-[8px] text-slate-400 uppercase font-black mb-1 text-center">F-F</label><div className="bg-slate-950 border border-white/10 rounded p-2 text-xs font-mono text-indigo-300 text-center font-bold">{th.fundamental.toFixed(3)}</div></div>
                                                        <div className="flex flex-col"><label className="text-[8px] text-slate-400 uppercase font-black mb-1 text-center">2-Tone</label><div className="bg-slate-950 border border-white/10 rounded p-2 text-xs font-mono text-indigo-300 text-center font-bold">{th.twoTone.toFixed(3)}</div></div>
                                                        <div className="flex flex-col"><label className="text-[8px] text-slate-400 uppercase font-black mb-1 text-center">3-Tone</label><div className="bg-slate-950 border border-white/10 rounded p-2 text-xs font-mono text-indigo-300 text-center font-bold">{th.threeTone.toFixed(3)}</div></div>
                                                    </>
                                                );
                                            })()}
                                        </div>
