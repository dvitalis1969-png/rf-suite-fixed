...
export type TabID = 'analyzer' | 'generator' | 'whitespace' | 'spectrum' | 'waterfall' | 'multiband' | 'talkback' | 'zonalTalkback' | 'multizone' | 'multistage' | 'siteMap' | 'festivalSiteMap' | 'multizoneSiteMap' | 'timeline' | 'festival' 
  | 'linkBudget' | 'audioTone' | 'fspl' | 'powerConverter' | 'fresnelZone' | 'antennaDownTilt' | 'cableLoss' | 'lineOfSight' | 'vswr' | 'imdDemo' | 'iemStudy' | 'diversityPlacement' | 'interference' | 'equipmentDatabase' | 'hardwareLink' | 'userGuide' | 'tourPlanning' | 'wmas' | 'plotGallery' | 'activityFeed' | 'frequencyForensics';
